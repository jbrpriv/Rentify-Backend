...
function _buildReceiptPDF(doc, payment, tenant, property, currencyCtx) {
  ...
  doc.save()
    .fillOpacity(1).font(FONT.regular).fontSize(7).fillColor('#93C5FD')
...
const generateAgreementPDF = async (agreement, landlord, tenant, property, res, options = {}) => {
  ...
  doc.pipe(res);
  _buildPDF(doc, agreement, landlord, tenant, property, currencyCtx);
...
const generateReceiptPDF = async (payment, tenant, property, res, options = {}) => {
  ...
  doc.pipe(res);
  _buildReceiptPDF(doc, payment, tenant, property, currencyCtx);
  doc.end();
};
