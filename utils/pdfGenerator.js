...
function finalizeClassicPagination(doc, title = 'RENTAL AGREEMENT') {
  ...
  for (let i = range.start + 1; i < range.start + range.count; i++) {
    doc.switchToPage(i);
    const savedY = doc.y;
    // Only repaint the narrow header area — NOT the full page (would erase content)
    doc.save().moveTo(PAGE.margin, 36).lineTo(PAGE.width - PAGE.margin, 36).lineWidth(1).strokeColor(C.gray900).stroke().restore();
    doc.save().font(FONT.bold).fontSize(9).fillColor(C.gray900)
      .text('RentifyPro', PAGE.margin, 22, { lineBreak: false }).restore();
    doc.save().font(FONT.regular).fontSize(8).fillColor(C.gray700)
    ...
    doc.save().font(FONT.regular).fontSize(7).fillColor(C.gray500)
      .text('Generated by RentifyPro. Digital signatures are legally binding.', 0, PAGE.height - 24, { align: 'center', lineBreak: false, width: PAGE.width }).restore();
    doc.y = savedY; // restore so PDFKit doesn't see an overflow and add a blank page
  }
  // Return to the last page so doc.y is sane when flushPages/end are called
...
function finalizeMinimalistPagination(doc, agreementId) {
  ...
  for (let i = range.start + 1; i < range.start + range.count; i++) {
    doc.switchToPage(i);
    const savedY = doc.y;
    // Only repaint the narrow header strip — NOT the full page (would erase content)
    doc.save().moveTo(0, 44).lineTo(PAGE.width, 44).lineWidth(0.4).strokeColor(C.gray300).stroke().restore();
    doc.save().font(FONT.bold).fontSize(7).fillColor(C.gray900)
      .text('RENTIFYPRO', PAGE.margin, 28, { lineBreak: false }).restore();
    doc.save().font(FONT.regular).fontSize(7).fillColor(C.gray500)
